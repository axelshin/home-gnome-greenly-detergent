window.addEventListener('DOMContentLoaded', function () {
  let dwarf1 = document.getElementsByClassName("gr-f-sc__dwarf_dwarf1")[0];
  let images = [];
  function preload() {
  ...
  //     "https://static.faberlic.com/fl_greenly/greenly_detergent/v2/app/images/dest/bg-other-sc.fone-sc5.jpg"
  // )
  preload(
      "https://static.faberlic.com/fl_greenly/greenly_detergent/v2/app/images/dest/bg-other-sc.jpg"
  )
...
let i = 0;
let currentSource = source[i];
let currentDest = dest[i];
...
function show() {
  ...
  if(now < len) {
    ...
    dwarf1.classList.add("dwarf1-anim2");
    let oldBottle = document.querySelector(".bottle1-rotate");
    if(oldBottle) {
      oldBottle.classList.remove("bottle1-rotate");
    }
    let newBottle = document.getElementsByClassName("gr-f-sc__product1")[1]; 
    newBottle.classList.add("bottle2-rotate");
...
let dwarf1 = document.getElementsByClassName("gr-f-sc__dwarf_dwarf1")[0];
if(dwarf1 && window.innerWidth > 500) {
  dwarf1.addEventListener("animationend", (e) => {
    let parent = currentDest.closest("div");
    parent.style.opacity = 1;
    setTimeout(() => {
      if(source && dest) {
        show_text();
    ...
    if(dwarf1.classList.contains("dwarf1-anim2")) {
      dwarf1.classList.add("anim-finish");
    }
  });
}

let scrolls = document.querySelectorAll(".scroll-block");
for(let scroll of scrolls) {
  if(scroll) {
    scroll.onwheel = function() { return false; }
  }
}
let btns = document.querySelectorAll(".gr-o-sc__left-bl_lists-show");
for(let btn of btns) {
  btn.onclick = ()=> {
    event.preventDefault();
    let list = btn.closest("div").querySelector("ul");
    if(list && !list.classList.contains("scrolled")) {
      list.classList.add("scrolled")
      btn.classList.add("rotate");
      let last = list.querySelector(".end-item");
      list.scrollTo({
...
let checkEl = document.querySelector("#title");
let point;
let check1 = true;
...
window.addEventListener("scroll", () => {
  ...
  if(checkEl && check1 && width > 500) {
    point = checkEl.getBoundingClientRect().top;
  } else if(check1 && width > 500) {
    checkEl = document.querySelector("#title");
  }
